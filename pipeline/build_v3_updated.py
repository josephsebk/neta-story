...
OUT_FILE = "neta_dataset_v3.json"

data = json.load(open(HOLDINGS_FILE))
prices = json.load(open(PRICES_FILE)); B = prices["baseline"]; C = prices["current"]
...
if need:
    ...
    json.dump({"baseline":B,"current":C},open(PRICES_FILE,"w"))
...
# Load existing fund NAVs
fund_base = {}
fund_cur = {}
if os.path.exists(FUND_NAVS_FILE):
    navs = json.load(open(FUND_NAVS_FILE))
    fund_base = navs.get("base", {})
    fund_cur = navs.get("cur", {})

def get_nav_hist(code):
    # Skip if we already have it to avoid redundant requests
    if code in fund_base and code in fund_cur and fund_base[code] is not None and fund_cur[code] is not None:
        return fund_base[code], fund_cur[code]
    try:
...
json.dump({"base":fund_base,"cur":fund_cur},open(FUND_NAVS_FILE,"w"))
...
fnav=json.load(open(FUND_NAVS_FILE))
# fund display names
FC2NAME={}
for d in data:
    for h in d.get("holdings",[]):
